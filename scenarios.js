...
export const SCENARIOS = {
    ...
    manufactorumRaid: {
        ...
        endPhase: (map) => {
            ...
            map.bombs.forEach((bomb, index) => {
                if (bomb.armed && !bomb.exploded) {
                    ...
                    map.log(`Bomb ${index + 1}: Rolled ${roll} + ${bomb.counter} counter = ${total}`);
        ...
        checkVictory: (map) => {
            ...
            if (explodedCount === totalBombs) {
                return {
                    ended: true,
                    winner: "attacker",
                    message: `ALL BOMBS DETONATED! Attacker Victory!<br>Reputation: +D3<br>Experience distributed.`
                };
            }
            ...
            if (attackers.length === 0) {
                return {
                    ended: true,
                    winner: "defender",
                    message: `Attackers eliminated! Defender Victory!<br>Credits: +${map.rand(2, 12) * 10}<br>Reputation: +D${explodedCount === 0 ? 6 : 3}`
                };
            }
        ...
        rewards: {
            ...
            defender: {
                victory: "2D6x10 credits, D6 Reputation (none exploded) or D3 Reputation (1-2 exploded)",
    ...
    conveyer: {
        ...
        setup: (map) => {
                ...
                for (let x = cx - r; x <= cx + r; x++) {
                    if (x >= 0 && x < map.width && y >= 0 && y < map.height) {
                        ...
                        if (dist <= r) {
                            ...
                            if (map.mapData[y][x].type === 'floor' || map.mapData[y][x].type === 'rubble') {
            ...
            while (placedCaskets < 4 && attempts < 100) {
                ...
                const distance = r + 3 + Math.random() * 9; // 3-12 cells from edge
            ...
            map.log(`Platform established. ${placedCaskets} loot caskets placed.<br>Platform will descend on 6+ (cumulative +1 per turn).`);
        },
        endPhase: (map) => {
            ...
            if (total >= 6) {
                // Platform moves!
                map.log('<span style="color: #ffff00; font-weight: bold;">*** PLATFORM DESCENDING! ***</span>');
                ...
                if (removedAttackers > 0) {
                    map.log(`${removedAttackers} attackers left behind as platform descends...`);
                }
        ...
        checkVictory: (map) => {
            ...
            if (map.round >= 9) {
                ...
                return {
                    ended: true,
                    winner: "defender",
                    message: `9 rounds complete! Defender Victory!<br>Loot recovered: ${onPlatform}/4 caskets<br>Credits: +${onPlatform * map.rand(1, 6) * 10}<br>Reputation: +D3`
                };
            }
            ...
            if (defenders.length === 0) {
                return {
                    ended: true,
                    winner: "attacker",
                    message: `All defenders eliminated! Attacker Victory!<br>Reputation: +D3`
                };
            }
        ...
        rewards: {
            attacker: {
                ...
                experience: "1 XP per fighter + 1 XP for being on platform when it moves"
            },
            defender: {
    ...
    fungalHorror: {
        ...
        setup: (map) => {
            map.log("SCENARIO: FUNGAL HORROR<br>A writhing nightmare spreads across the battlefield!");
            ...
            map.log(`Central fungal horror established at [${centerMarker.x},${centerMarker.y}].<br>Overgrown areas cause Blaze-like damage!<br>Line of sight limited to 6" in overgrowth.<br>Game ends at ${map.maxFungalMarkers} markers or no fighters remain.`);
        },
        endPhase: (map) => {
            ...
            map.fungalMarkers.forEach((marker, index) => {
                ...
                if (roll >= 4) {
                    ...
                    map.log(`<span style="color: #adff2f; font-weight: bold;">Fungus SPREADS to [${clampedX},${clampedY}]!</span>`);
                }
            });
            ...
            if (map.fungalMarkers.length >= map.maxFungalMarkers) {
                map.log(`<span style="color: #ff0000; font-weight: bold;">CRITICAL: ${map.fungalMarkers.length} fungal markers! Board is overrun!</span>`);
            }
            ...
            if (map.fungalMarkers.length >= 9) {
                ...
                if (attackers.length > 0 && defenders.length === 0) {
                    return {
                        ended: true,
                        winner: "attacker",
                        message: "Fungal horror overruns battlefield! Attackers survive!<br>Reputation: +D3"
                    };
                } else if (defenders.length > 0 && attackers.length === 0) {
                    return {
                        ended: true,
                        winner: "defender",
                        message: "Fungal horror overruns battlefield! Defenders survive!<br>Reputation: +D3"
                    };
                } else if (attackers.length > 0 || defenders.length > 0) {
                    return {
                        ended: true,
                        winner: attackers.length >= defenders.length ? "attacker" : "defender",
                        message: `Fungal horror overruns battlefield! ${attackers.length >= defenders.length ? "Attackers" : "Defenders"} survive with more fighters!<br>Reputation: +D3`
                    };
                } else {
                    return {
                        ended: true,
