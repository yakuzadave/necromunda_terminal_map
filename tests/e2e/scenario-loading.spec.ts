...
test.describe('Scenario Loading', () => {
  ...
  test('should display legend with all terrain types', async ({ page }) => {
    ...
    await expect(page.locator('.legend-item:has-text("Marauder (Attacker)")')).toBeVisible();
    await expect(page.locator('.legend-item:has-text("Garrison (Defender)")')).toBeVisible();
    await expect(page.locator('.legend-item:has-text("Bulkhead Structure")')).toBeVisible();
  });
...
test.describe('Scenario Generation', () => {
  ...
  test('should generate random ambush scenario', async ({ page }) => {
    ...
    const hasAmbushScenario =
      text?.includes('BUSHWHACK') ||
      text?.includes('SCRAG') ||
